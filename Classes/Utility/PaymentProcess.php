...
{
    const PAYPAL_API_SANDBOX = 'https://www.sandbox.ideal.com/webscr?';
    const PAYPAL_API_LIVE = 'https://www.ideal.com/webscr?';
    ...
    protected function getTypoScript()
    {
        ...
        $this->conf = $this->typoScriptService->convertTypoScriptArrayToPlainArray(
            $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_cartideal.']
        );
    ...
    {
        $idealUrl = $this->getIdealUrl();

        $ch = curl_init($idealUrl);
        if ($ch == false) {
            return false;
        }
     ...
     * @return string
     */
    protected function getIdealUrl(): string
    {
