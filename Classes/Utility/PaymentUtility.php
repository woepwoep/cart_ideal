...
{
    const IDEAL_API_SANDBOX = 'https://www.sandbox.ideal.com/webscr?';
    const IDEAL_API_LIVE = 'https://www.ideal.com/webscr?';
    ...
    public function handlePayment(array $params): array
    {
        ...
        if ($this->orderItem->getPayment()->getProvider() === 'IDEAL') {
    ...
    protected function getQueryUrl(): string
    {
        if ($this->conf['sandbox']) {
            return self::IDEAL_API_SANDBOX;
        }

        return self::IDEAL_API_LIVE;
    }
    ...
    protected function getQueryFromCart()
    {
        ...
        if ($this->conf['sendEachItemToIdeal']) {
    ...
    protected function addEntireCartToQuery()
    {
        ...
        $this->paymentQuery['item_name_1'] = $this->conf['sendEachItemToIdealTitle'];
